...
write.csv(cor_data, file = "cor_data.csv", row.names = FALSE)
write.csv(cor_data_IS, file = "cor_data_IS.csv", row.names = FALSE)
write.csv(cor_data_IR, file = "cor_data_IR.csv", row.names = FALSE)
...
plot(
  as.numeric(nutrition_expression_data["VitC", sample_info_IR$sample_id]),
  as.numeric(metabolomics_expression_data["nRPLC_441.3947_11.3", sample_info_IR$sample_id])
)
...
plot(
  ...
  as.numeric(metabolomics_expression_data["nRPLC_441.3947_11.3", sample_info_IS$sample_id])
)
...
plot
ggsave(plot, filename = "all_cor_all_metabolite.pdf", width = 10, height = 10)
...
plot = 
  ...
  plot = ggplotify::as.ggplot(plot)
plot
# ggsave(plot, filename = "all_cor.pdf", width = 10, height = 10)
...
temp_cor_is <-
  ...
  tidyr::pivot_longer(cols = -nutrition, names_to = "metabolite", values_to = "cor")

temp_p_is <-
  normal_p  %>% 
  tibble::rownames_to_column(var = "nutrition") %>%
  tidyr::pivot_longer(cols = -nutrition, names_to = "metabolite", values_to = "p")


temp_is <-
  temp_cor_is  %>% 
  dplyr::left_join(temp_p_is, by = c("nutrition", "metabolite"))
...
temp_ir <-
  temp_cor_ir  %>% 
  dplyr::left_join(temp_p_ir, by = c("nutrition", "metabolite"))
...
# 
# idx <-
#   which(cor_data_IR_output$p_adjust < 0.05 &
#           abs(cor_data_IR_output$cor) > 0.5)
# ir_idx <- idx
...
#       abs(cor_data_IS_output$cor) > 0.5)
# 
# idx <-
#   which(cor_data_IS_output$p_adjust < 0.05 &
#           abs(cor_data_IS_output$cor) > 0.5)
# 
# is_idx <- idx
# 
...
# 
# ir_metabolite <- unique(cor_data_IR_output$Metabolite[ir_idx])
# is_metabolite <- unique(cor_data_IS_output$Metabolite[is_idx])
# 
# hmdb_id <-
#   hmdb_id[hmdb_id != ""] %>%
#   stringr::str_split("\\|") %>%
